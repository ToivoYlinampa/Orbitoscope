...
// Define laser index to ignore
const int IGNORE_LASER_INDEX = 8;  // 8th index corresponds to the 9th laser
...
{ 
  if (Serial.available() > 0) //if something comes from the computer
  {
    ...
    if (newDataX == true) //we only enter this long switch-case statement if there is a new command from the computer
    {
      ...
      {
        case 'f': // Turn relay on, powering the camera
                digitalWrite(relayPin, HIGH);
                Serial.println("Camera ON.");
                break;

            case 'F': // Turn relay off, cutting power to the camera
                digitalWrite(relayPin, LOW);
...
void checkLasersAndDetectors() {
    ...
    for (int i = 0; i < NUM_LASERS; i++) {
        if (i == IGNORE_LASER_INDEX) {
            Serial.print("Laser ");
            Serial.print(i + 1);
            Serial.println(" is ignored.");
            continue;  // Skip this laser
        }
        ...
        // Invert logic for specific lasers if needed
        if (i == 0 || (i >= 5 && i <= 9)) {
...
bool checkAllLasers() {
    return true; // Always return true, effectively ignoring laser checks.
}
